...
ValidationDefaultOptions.prototype = {
	...
	showErrorMsg:true//是否通过span提示错误信息，否则通过alert提示错误信息。
}
...
['validation-failed' , 'Validation failed.'],
	...
	['validate-phone','请输入正确的电话号码,如:010-29392929,或者5-8位有效数字，当前长度为%s.'],
	...
	ValidationUtils = {
		...
		all: function(collection,iterator) {
				 ...
				 $.each(collection,function(index, value) {
					 result = result && !!(iterator || function(x){return x;})(index,value );
					 return result;
				 });
				 return result;
			 },
		...
		getFormElements: function(form){
					rinput = /^(?:color|date|file|radio|checkbox|datetime|datetime-local|email|hidden|month|number|password|range|search|tel|text|time|url|week)$/i;
					...
					return form.map(function(){
								...
								.filter(function(){
									...
									var x =  this.name && !this.disabled &&
										( this.checked || rselectTextarea.test( this.nodeName ) ||
											rinput.test( this.type ) );
                            try {
                            //console.log('result: ' + x + ', elm:' + this.outerHTML);
                            } catch (e) {}
                            return x;
								});
			}
...
Validator.prototype = {
	...
	error : function(v,elm,useTitle) {
                ...
                return (elm && elm.attr("title")) ? (elm.attr("title").charAt(0)=='!' ? elm.attr("title").substring(1, elm.attr("title").length) : (elm.attr("title") + error)) : error;
...
Validation.prototype = {
	...
	initialize : function(form, options){
					 ...
					 if(this.options.immediate) {
						 ...
						 for(var i = 0; i < elements.length; i++) {
                             ...
                             var bindMethod = (input.is(':radio') || input.is(':checkbox')) ? 'click' : 'blur';
                             if (input.is('select')) bindMethod = 'change';
                             input.bind(bindMethod, function(ev) {
	...
	validate : function() {
				 ...
				 var useTitles = this.options.useTitles;
				 var callback = this.options.onElementValidate;
				 var sem = this.options.showErrorMsg;
				 var elements = ValidationUtils.getFormElements(this.form);
				 if(this.options.stopOnFirst) {
					for(var i = 0; i < elements.length; i++) {
						   var elm = $(elements[i]);
						   result = Validation.validateElement(elm,{useTitle : useTitles, onElementValidate : callback,showErrorMsg:sem});
						   if(!result) break;
					   }
				   } else {
					   for(var i = 0; i < elements.length; i++) {
						   var elm = $(elements[i]);
						   if(!Validation.validateElement(elm,{useTitle : useTitles, onElementValidate : callback,showErrorMsg:sem})) {
							   result = false;
						   }
					   }
				   }
				   //将焦点移动到校验失败的地方
				   if(!result && this.options.focusOnError && this.options.showErrorMsg) {
					   var first = $.grep(ValidationUtils.getFormElements(this.form),function(elm){return $(elm).hasClass('validation-failed')})[0];
					   if(first.select) first.select();
					   first.focus();
                       $("html,body").animate({scrollTop: $(first).offset().top - 20}, 100);
				   }
...
$.extend(Validation, {
	...
	validateElement : function(elm, options){
						  ...
						  for(var i = 0; i < cn.length; i++) {
							  var className = cn[i];
							  var test = Validation.test(className,elm,options.useTitle,options.showErrorMsg);
	...
	newErrorMsgAdvice : function(name,elm,errorMsg) {
							var advice = '<span class="error" id="advice-' + name + '-' + ValidationUtils.getElmID(elm) +'" style="display:none">&nbsp;&nbsp;' + errorMsg + '</span>';
							var tagName = '';
	...
	test : function(name, elm, useTitle,sem) {
			   ...
			   if(errorMsg) {
                   ...
                   Validation.showErrorMsg(v.className,elm,errorMsg,sem);
				   return false;
			   } else {
	...
	getAdvice : function(name, elm) {
                    ...
                    // 服务器返回的错误信息提示
                    advice = $('#advice-server-' + ValidationUtils.getElmID(elm));
                    if(advice && advice[0]){
                        return advice;
                    }
					return undefined;
...
Validation.addAllThese([
		...
		['validate-one-required', function (v,elm) {
			var p = elm.parent();
			var options = $(':input',p);
			return ValidationUtils.any(options,function(val, elm) {
				return $(elm).filter(':checked').length > 0;
			});
		},{ignoreEmptyValue : false}],
		...
		Validation.addAllThese([
				 ...
				 */
				['validate-file',function(v,elm,args,metadata) {
					return ValidationUtils.any($.makeArray(args),function(i,extentionName) {
				...
				['validate-phone',/^((0[1-9]{3})?(0[12][0-9])?[-])?\d{5,8}$/],
				...
				Validation.autoBind = function() {
					...
					//绑定查询form，查询form不使用span提示，改为alert提示
					var queryform = $("#queryForm");
					var validationQuery = new Validation(queryform,{immediate:false,showErrorMsg:false});
				};
//初始化
$(Validation.autoBind);
...
var isIE7 = navigator.userAgent.indexOf('MSIE 7.') != -1 && !isIE8;
...
var $topWindow = function () {
    ...
    */
    return parentWin;
	//return window.parent.frames["content-iframe"]; 
};
...
var Dialog = function () {
    ...
    this.MessageIcon = "icon_alert.gif";
    this.MessageTitle = "";
    this.Message = "";
...
Dialog.prototype.create = function () {
    ...
    var html = '\
  <table class="dialog_table" id="_DialogTable_' + this.ID + '" width="' + (this.Width + 26) + '" cellspacing="0" cellpadding="0" border="0" onselectstart="return false;" style="-moz-user-select: -moz-none; font-size:12px; line-height:1.4;border-collapse: collapse;">\
    ...
    <tr valign="top">\
      ...
      <td align="center"><table width="100%" cellspacing="0" cellpadding="0" border="0" bgcolor="#ffffff">\
          <tr id="_MessageRow_' + this.ID + '" style="' + (this.ShowMessageRow ? "" : "display:none") + '">\
            <td valign="top" height="35"><table width="100%" cellspacing="0" cellpadding="0" border="0" style="background:#eaece9 url(' + IMAGESPATH + 'dialog_bg.jpg) no-repeat scroll right top;" id="_MessageTable_' + this.ID + '">\
                <tr>\
                  <td width="50" height="35" align="center"><img width="32" height="32" src="' + IMAGESPATH + this.MessageIcon + '" id="_MessageIcon_' + this.ID + '"/></td>\
	...
	' + (function (obj) {
        ...
        if (obj.URL) return '<h1 id="_DialogFrameLoading_'+ this.ID+'" style="background-color: #000;color:#fff;opacity:.5" >正在加载....</h1><iframe width="100%" height="100%" frameborder="0" style="display:none;border:none 0;" allowtransparency="true" id="_DialogFrame_' + obj.ID + '" src="' + obj.displacePath() + '"></iframe>';
        return "";
    })(this) + '\
              </div></td>\
          </tr>\
          <tr id="_ButtonRow_' + this.ID + '" style="' + (this.ShowButtonRow ? "" : "display:none") + '">\
            <td height="36"><div id="_DialogButtons_' + this.ID + '" style="border-top: 1px solid #DADEE5; padding: 8px 20px; text-align: right; background-color:#f6f6f6;">\
                <input type="button" class="dialog_btn" value="确 定" id="_ButtonOK_' + this.ID + '"/>\
                <input type="button" class="dialog_btn_cancel" value="取 消" onclick="Dialog.getInstance(\'' + this.ID + '\').close();" id="_ButtonCancel_' + this.ID + '"/>\
              </div></td>\
          </tr>\
        </table></td>\
    ...
    if (this.URL) {
        ...
        innerFrameOnload = function () {
            try {
                ...
                self.innerFrame.style.display="";
                if(topWin.$id("_DialogFrameLoading_" + this.ID)){
                	topWin.$id("_DialogFrameLoading_" + this.ID).style.display="none";
                }
            } catch(e) {
                if (window.console && window.console.log) console.log("可能存在访问限制，不能获取到弹出窗中的文档对象。");
            }

...
$.openLayer = function(p){
	var param = $.extend({
		...
		url : "",						//ajax请求url
		pid : "",						//父id
		shared : false,					//如果页面中有多于1个的弹出选择,是否共享缓存数据
	...
	var fs = {
		init_Container : function(){	$("#bodybg").remove();//初始化头部和内容容器
			//标题
			var TITLE = param.title + ",点击机构名称选择下级区域";
		...
		init_area : function(){			//初始化数据容器
			...
			//已选择项容器
			var selArea = $("<div id='selArea'><div>已选择机构：</div></div>");
			...
			if((! jQuery.isEmptyObject(param.selValues) && param.selValues.length > 0)){
				...
				$.each(param.selValues,function(idx,obj){
					...
					html.push("<input type='checkbox' checked='true' value='" + obj.id + "'>");
			...
			if (jQuery.isEmptyObject(param.selValues)){ selArea.hide(); }
		...
		//	if(param.cacheEnable){ data = _cache[pid];}	//如果cache开启则首先从cache中取得数据
			...
			//if ((data == null || data == "" || data.length == 0) &&  url != "" ){alert();
				$.ajax({
					type : "post",						//post方式
					url : url,							//ajax查询url
		...
		//	}
			...
			for(var i = 0 ; i < dat.length ; i++){
				...
				region = dat[i];
				html.push("<span title='"+region.name+"' name='"+pid+"' style='width:"+span_width+"px;white-space:nowrap;float:left;overflow:hidden'>");
				//flag等于1，无法选择省。
				if(flag==1){
					if(region.pId =='0'){
						html.push("<input type='checkbox' disabled='disabled' areaCode='"+region.areaCode+"' value='" + region.id + "'>");
					}else{
						html.push("<input type='checkbox' areaCode='"+region.areaCode+"' value='" + region.id + "'>");
					}
				}else{
					html.push("<input type='checkbox' areaCode='"+region.areaCode+"' value='" + region.id + "'>");
				}
				html.push("<span name='"+targetid.attr("id")+"' style='margin-left:"+inspan_width+"px;cursor:pointer'>" + region.name+ "</span>");
				html.push("</span>");
			}

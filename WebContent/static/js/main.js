...
function selectSinggleGood(selectedItems,callback,opts){
	...
	showDialog("选择商品", G_CTX_ROOT + "/item/item/selectSingleItem?ids=" + selectedItems ,function(innerDoc,innerWin){
		...
		if(!jQuery.isEmptyObject(selectRadio)){
			...
			if(jQuery.isFunction(callback)){
				callback(valId,valTxt,innerDoc,innerWin);
			}
		}
	},param);
}
...
function selectRegion(selector,returnValueId,returnTxtId,pid,opts){ 
	...
	$( selector).click(function(){
		...
		var param = $.extend({
			...
			pid : $("#"+pid).val(),
			title:'部门选择',
			url: G_CTX_ROOT + '/sys/unit/selectUnit',
 ...
 * 
 */
function selectItemType(flag,selector,callback,opts){
	//判断容器是否存在
	...
	var setting = {
			...
			async: {
				enable: true,
				url:  G_CTX_ROOT + '/sys/type/selectItemType/'+flag,
				dataType : "json",
				autoParam: ["id"],
 ...
 * 选择人员
 * @param unitIdId 单位编号
 * @param selectedItems 已选的人员
 * @param callback 回调
 * @param opts 
 */
function selectUser(unitId,selectedItems,callback,opts){
	var param = $.extend({
		Height: 400,
		Width : 900
	},opts||{});
	
	showDialog("选择人员", G_CTX_ROOT + "/sys/user/selectuser?unitId="+unitId+"&ids=" + selectedItems ,function(innerDoc,innerWin){
		var ids = [];
		var txts = [];
		$("input[name='id']:checked",innerDoc).each(function(){
			ids.push($(this).val());
			txts.push($(this).attr("userName"));
		});
		if(jQuery.isFunction(callback)){
			callback(ids,txts,innerDoc,innerWin);
		}
 ...
 * @param opts 
 */
function selectJoinDept(unitId,selectedItems,callback,opts){
	var param = $.extend({
		Height: 400,
		Width : 900
	},opts||{});
	
	showDialog("选择单位", G_CTX_ROOT + "/sys/unit/selectjoindept?unitId="+unitId+"&ids=" + selectedItems ,function(innerDoc,innerWin){
		var ids = [];
		var txts = [];
		$("input[name='id']:checked",innerDoc).each(function(){
			ids.push($(this).val());
